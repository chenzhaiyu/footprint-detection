...
use Mask-RCNN to get patches, then use closed form matting to refine them
...
for file_name in file_names:
    ...
    for i in range(0, N):
        ...
        trimap = np.zeros(image.shape[:2], np.uint8)  # initialize all sure background

        trimap[y1:y2, x1:x2] = 128

        trimap[rcnnmask == True] = 255

        # run closed form matting
        alpha = closed_form_matting.closed_form_matting_with_trimap(image / 255.0, trimap / 255.0)
